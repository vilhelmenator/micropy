...
static inline void _nr_time_init(void) { mach_timebase_info(&_mp_tb); }
static inline uint64_t _nr_time_ns(void) {
    return (mach_absolute_time() * _mp_tb.numer) / _mp_tb.denom;
}
#elif defined(_WIN32)
#  include <windows.h>
static LARGE_INTEGER _mp_qpf;
static inline void _nr_time_init(void) { QueryPerformanceFrequency(&_mp_qpf); }
static inline uint64_t _nr_time_ns(void) {
...
#else
#  include <time.h>
static inline void _nr_time_init(void) {}
static inline uint64_t _nr_time_ns(void) {
...
static inline void _nr_cprint(const char *color, const char *fmt, ...) {
...
#define _NR_GREEN "\033[32m"
#define _NR_RED   "\033[31m"

/* ---- Global test state ---- */
static int _nr_test_failures   = 0;
static int _nr_test_total      = 0;
static int _nr_test_fail_total = 0;

/* ---- Assertion macros ---- */
#define nr_test_assert(cond) do { \
    if (!(cond)) { \
        _nr_test_failures++; \
...
#define nr_test_assert_msg(cond, msg) do { \
    if (!(cond)) { \
        _nr_test_failures++; \
...
#define nr_test_assert_eq(a, b) do { \
    if ((a) != (b)) { \
        _nr_test_failures++; \
